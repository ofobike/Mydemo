...
app.controller('goodsController', function ($scope, $controller, $location, uploadService, goodsService, itemCatService, typeTemplateService) {
     ...
     * @param status
     */
    $scope.updateStatusMarket=function(status){

        /*if ("1"==status){
            alert("商品上架:"+status);
            alert("要传入的商品Id为:"+$scope.selectIds);
        }else {
            alert("商品上架:"+status);
            alert("要传入的商品Id为:"+$scope.selectIds);
        }
*/
        //调用服务层
        goodsService.updateMarketStatus($scope.selectIds,status).success(function(response){
